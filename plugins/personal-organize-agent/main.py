"""个人整理Agent插件：笔记归纳、资料整理、信息分类汇总（ReAct 单循环图）。

- think 节点：携带可用工具清单请求 LLM，流式输出（运行时自动转 agent:think）；
- 需要读取用户上传资料时调用 demo-file-tool:read_text（path 如 data/uploads/xxx.txt）；
- 整理输出为结构化 Markdown；用户要求保存时调用 file-write-tool:write_text 写入 data/generated_docs/；
...
class PersonalOrganizeAgentPlugin(BaseAgentPlugin):
    system_prompt = (
        "你是 NvwaAgent 个人整理智能体，一位细致的个人整理助手，"
        "擅长笔记归纳、资料整理与信息分类汇总。\n"
        "工作流程：\n"
        "1. 需要读取用户上传的资料时，调用工具 demo-file-tool:read_text"
        "（path 如 data/uploads/xxx.txt）；\n"
        "2. 整理结果输出为结构化 Markdown：使用清晰的标题、分组与要点列表；\n"
        "3. 用户要求保存整理结果时，调用工具 file-write-tool:write_text 写入 data/generated_docs/ 目录，"
        "文件名需语义化（如 notes-整理-日期.md）；\n"
        "4. 整理内容必须忠于原文，不添加原文没有的信息。\n"
...
def on_load(ctx: PluginContext) -> BaseAgentPlugin:
    ctx.logger.info("个人整理Agent插件加载")
    return PersonalOrganizeAgentPlugin()


def on_activate(ctx: PluginContext) -> None:
    ctx.logger.info("个人整理Agent插件激活")


def on_deactivate(ctx: PluginContext) -> None:
    ctx.logger.info("个人整理Agent插件禁用")


def on_unload(ctx: PluginContext) -> None:
    ctx.logger.info("个人整理Agent插件卸载")
