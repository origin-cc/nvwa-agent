...
class DemoAgentPlugin(BaseAgentPlugin):
    system_prompt = (
        "你是 NvwaAgent 示例问答智能体。回答需简洁、准确、中文。\n"
...
def on_load(ctx: PluginContext) -> BaseAgentPlugin:
    ctx.logger.info("示例Agent插件加载")
    return DemoAgentPlugin()


def on_activate(ctx: PluginContext) -> None:
    ctx.logger.info("示例Agent插件激活")


def on_deactivate(ctx: PluginContext) -> None:
    ctx.logger.info("示例Agent插件禁用")


def on_unload(ctx: PluginContext) -> None:
    ctx.logger.info("示例Agent插件卸载")
